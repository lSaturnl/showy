;(function () {
	...
	function component(object) {
		...
		this.rch = function (json, noreset) {
			...
			if (typeof signalR == 'undefined') {
				Lampa.Utils.putScript(
					['http://89.110.72.185:9118/signalr-6.0.25_es5.js'],
					function () {},
					false,
					function () {
						load()
		...
		this.externalids = function () {
			return new Promise(function (resolve, reject) {
				if (!object.movie.imdb_id || !object.movie.kinopoisk_id) {
					...
					var url =
						Defined.localhost + 'externalids?' + query.join('&')
					...
					if (!red) {
						var _filter = json.online.filter(function (c) {
							return any
								? c.show
								: c.show &&
										c.name.toLowerCase() == last_balanser
						})
						if (_filter.length) {
							red = true
							resolve(
					...
					_this5.getFileUrl(
						...
						function (json, json_call) {
							if (json && json.url) {
								...
								first.url = json.url
								first.headers =
									json_call.headers || json.headers
								first.quality =
									json_call.quality || item.qualitys
								first.hls_manifest_timeout =
									json_call.hls_manifest_timeout ||
									json.hls_manifest_timeout
								...
								if (first.url) {
									...
									if (element.url && element.isonline) {
										...
										if (
											...
											Noty.show(
												'Р’РёРґРµРѕ РѕС‚РєСЂС‹С‚Рѕ РІ playerInner',
												{ time: 3000 }
											)
											$.get(
												'http://89.110.72.185:9118/player-inner/' +
													element.url
											)
											return
										}
				...
				if (
					...
					if (videos.length) {
						if (buttons.length) {
							...
							var select_voice_url =
								this.getChoice(balanser).voice_url
							var select_voice_name =
								this.getChoice(balanser).voice_name
		...
		this.similars = function (json) {
			...
			json.forEach(function (elem) {
				...
				elem.time = elem.time || ''
				elem.info = info.join(
					'<span class="online-prestige-split">в—Џ</span>'
				)
		...
		this.updateWatched = function () {
			...
			if (watched) {
				...
				body.append(
					'<span>' +
						Lampa.Lang.translate('lampac_no_watch_history') +
						'</span>'
				)
		}
		/**
			...
			this.getEpisodes(items[0].season, function (episodes) {
				...
				items.forEach(function (element, index) {
					...
					var voice_name =
						choice.voice_name ||
						(filter_find.voice[0]
							? filter_find.voice[0].title
							: false) ||
						element.voice_name ||
						(serial ? 'РќРµРёР·РІРµСЃС‚РЅРѕ' : element.text) ||
						'РќРµРёР·РІРµСЃС‚РЅРѕ'
					if (element.quality) {
						element.qualitys = element.quality
						element.quality = Lampa.Arrays.getKeys(
							element.quality
						)[0]
					}
					...
					if (serial && !episode) {
						image.append(
							'<div class="online-prestige__episode-number">' +
								('0' + (element.episode || index + 1)).slice(
									-2
								) +
								'</div>'
						)
					...
					element.mark = function () {
						...
						if (viewed.indexOf(hash_behold) == -1) {
							...
							if (
								html.find('.online-prestige__viewed').length ==
								0
							) {
								html.find('.online-prestige__img').append(
									'<div class="online-prestige__viewed">' +
		...
		this.noConnectToServer = function (er) {
			...
			html.find('.online-empty__buttons').remove()
			html.find('.online-empty__title').text(
				Lampa.Lang.translate('title_error')
			)
			html.find('.online-empty__time').text(
				er && er.accsdb
					? er.msg
					: Lampa.Lang.translate(
					  ...
					  ).replace('{balanser}', balanser[balanser].name)
			)
			scroll.clear()
			scroll.append(html)
			this.loading(false)
							...
							status.onComplite = function (result) {
								...
								keys.forEach(function (name) {
									...
									if (
										...
										var cards = line.data.map(function (
											item
										) {
											item.title =
	...
	function startPlugin() {
		...
		var manifst = {
			...
			name: 'Showy',
			description:
				'РџР»Р°РіРёРЅ РґР»СЏ РїСЂРѕСЃРјРѕС‚СЂР° РѕРЅР»Р°Р№РЅ СЃРµСЂРёР°Р»РѕРІ Рё С„РёР»СЊРјРѕРІ',
			component: 'showy',
		...
		var button =
			'<div class="full-start__button selector view--online_showy showy--button" data-subtitle="'
				.concat(manifst.name, ' v')
				.concat(
					manifst.version,
				...
				) // РЅСѓР¶РЅР° Р·Р°РіР»СѓС€РєР°, Р° С‚Рѕ РїСЂРё СЃС‚СЂР°С‚Рµ Р»Р°РјРїС‹ РіРѕРІРѕСЂРёС‚ РїСѓСЃС‚Рѕ
		...
		if (event.name == 'activity') {
			if (Lampa.Activity.active().component == 'showy') {
				var add_ads = setInterval(function () {
					if (
						document.querySelector('.online-prestige-watched') !==
						null
					) {
			...
			if (Lampa.Activity.active().component == 'full') {
				var add_ads2 = setInterval(function () {
					var titleText = $('.selectbox__title').text() // РЎРѕР·РґР°РµРј РїРµСЂРµРјРµРЅРЅСѓСЋ РґР»СЏ С‚РµРєСЃС‚Р°

					if (
						titleText.length &&
						titleText === 'РСЃС‚РѕС‡РЅРёРє' &&
						Lampa.Activity.active().component === 'full'
					) {
						// РџСЂРѕРІРµСЂСЏРµРј РґР»РёРЅСѓ РїРµСЂРµРјРµРЅРЅРѕР№ Рё РµРµ Р·РЅР°С‡РµРЅРёРµ
